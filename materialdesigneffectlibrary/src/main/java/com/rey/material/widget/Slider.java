 ...
 * Modified method setPosition(), now listener can tell whether the event is an ACTION_UP
 */
public class Slider extends View implements ThemeManager.OnThemeChangedListener {
	...
	public interface OnPositionChangeListener {
		 ...
		 * @param action_up
		 */
		void onPositionChanged(Slider view, boolean fromUser, float oldPos, float newPos, int oldValue, int newValue, boolean action_up);
	}
	...
	public void setValueRange(int min, int max, boolean animation) {
		...
		if (mOnPositionChangeListener != null && oldPosition == getPosition() && oldValue != getExactValue())
			mOnPositionChangeListener.onPositionChanged(this, false, oldPosition, oldPosition, Math.round(oldValue), getValue(), false);
	}
	...
	private void setPosition(float pos, boolean moveAnimation, boolean transformAnimation, boolean fromUser) {
		setPosition(pos, moveAnimation, transformAnimation, fromUser, false);
	}

	private void setPosition(float pos, boolean moveAnimation, boolean transformAnimation, boolean fromUser, boolean action_up) {
...
//		if (change && mOnPositionChangeListener != null)
			mOnPositionChangeListener.onPositionChanged(this, fromUser, oldPos, newPos, oldValue, newValue, action_up);
	}
	...
	public boolean onTouchEvent(@NonNull MotionEvent event) {
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_UP:
				if (mIsDragging) {
					mIsDragging = false;
					setPosition(getPosition(), true, true, true, true);

					if (getParent() != null)
						getParent().requestDisallowInterceptTouchEvent(false);
				} else if (distance(mMemoPoint.x, mMemoPoint.y, x, y) <= mTouchSlop) {
					float position = correctPosition(Math.min(1f, Math.max(0f, (x - mDrawRect.left) / mDrawRect.width())));
					setPosition(position, true, true, true, true);
				}
				break;
